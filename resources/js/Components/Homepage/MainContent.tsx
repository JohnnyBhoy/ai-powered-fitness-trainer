


const MainContent = () => {
    return (
        <div className="flex justify-center items-center px-4 py-10">
            <div className="bg-[#23B5D3] text-white p-10 rounded-2xl shadow-sm flex items-center max-w-5xl w-full">
                {/* Left Content */}
                <div className="w-1/2 px-6 pl-15">
                    <h1 className="text-xl font-bold flex items-center mb-4">
                        {/* <img src="/logo.png" alt="Go Peak Fit" className="h-6 mr-2" /> */}
                    </h1>
                    <h2 className="text-4xl md:text-3xl font-alfarn font-extrabold mb-4 leading-tight">
                            TRANSFORM YOUR<br />
                            <span className="text-white">HEALTH—FOREVER</span><br />
                            <span>
                                <p className="text-gray-800 text-3xl font-extrabold">Start with the $1
                                Challenge</p>
                            </span>
                    </h2>
                    <p className="text-xs md:text-xs mb-4 text-gray-900 font-medium">
                        You say you want results. You say you want change. But are you
                        actually willing to do what it takes? This isn’t a detox. This isn’t a
                        gimmick. This is your wake-up call.
                    </p>
                    <button className="px-6 py-3 bg-white text-black rounded-full text-lg font-medium hover:bg-gray-100 transition text-[10px] font-arial">
                        Start Your $1 Challenge Now
                    </button>
                </div>

                {/* Right Image */}
                <div className="w-1/2 flex justify-center relative">
                    <img src="/images/main-img.png" alt="Fitness Image" className="rounded-xl h-80" />
                
                    {/* Workout Notification Image */}
                    <img 
                        src="/images/main-subimg1.png" 
                        alt="Workout Notification Image" 
                        className="rounded-xl h-12 absolute top-30 left-32 transform -translate-x-1/2"
                    />

                    {/* Rating Image*/}
                    <img 
                        src="/images/main-subimg2.png" 
                        alt="Rating Image" 
                        className="rounded-xl h-12 absolute bottom-20 right-0 transform -translate-x-1/2"
                    />
                </div>

            </div>
        </div>
    );
};

export default MainContent;
