...
function GenerateWeeklyNutrition({ userId }: { userId: number }) {
    ...
    const generateWeeklyNutrition = async (): Promise<void> => {
        if (!userId) {
        ...
        try {
            const response = await axios.post(GENERATE_WEEKLY_NUTRITION_URL, {
                user_id: userId,
            });
            ...
            if (response.status === 200 && data?.success) {
                toast.success(data?.message || "Weekly nutrition plan generated successfully!");
                console.log("Generated plan:", data?.program ?? data);
                setWeeklyPrograms(data?.program);
            } else {
                toast.warning(data?.message || "Something went wrong while generating the plan.");
            }
        } catch (error: any) {
            ...
            if (error.response?.status === 422) {
                ...
                toast.error("Failed to generate plan. Please try again later.");
            }
        } finally {
            setLoading(false);
...
export default GenerateWeeklyNutrition